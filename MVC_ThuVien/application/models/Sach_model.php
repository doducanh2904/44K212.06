...
//------------XỬ LÝ CÁC CHỨC NĂNG QUẢN LÝ ĐỘC GIẢ ---------------------------------------
	...
	{
		$data = array('id_docgia' =>$id ,'ten_docgia'=>$ten,'donvi_docgia'=>$donvi,'ngaylamthe_docgia'=>$ngaylamthe);
		$this->db->where('id_docgia', $id);
	   if($this->db->update('doc_gia', $data))
	   {
	   	...
	   	echo '<script>
				...
				return $this->db->update('doc_gia', $data);
	   }
	   else
	   {
	   	echo '<script>
...
//---------------------------- XỬ LÝ CHỨC NĂNG TÌM KIẾM------------------------------------------- 
   ...
   //	$this->db->join('danh_muc', 'sach.id_danhmuc = danh_muc.id_danhmuc',);
   	...
   	//$this->db->join('danh_muc', 'sach.id_danhmuc = danh_muc.id_danhmuc');
    //$this->db->like('ten_danhmuc', $i, 'after');
    $this->db->or_like('ten_docgia', $id, 'after');
   	$data=$this->db->get();
   ...
   public function searchCho_model($id)
   {
  	...
  	$this->db->select('*');
   	...
   	$this->db->like('id_docgia', $id, 'after');
    $this->db->or_like('id_sach', $id, 'after');
   	$data=$this->db->get();
   	  $data = $data->result_array();
   	 // var_dump($data);
   return $data;
